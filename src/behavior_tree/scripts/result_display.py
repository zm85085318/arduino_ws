...
class RecognitionDisplay:
    ...
    def __init__(self):
        ...
        self.position_listener = TransformListener()
    ...
    def detectionsCallback(self, received_values):
        for count in range(len(received_values.bounding_boxes)):
            ...
            if target_class == "person" and target_probability > 0.85:
                self.position_flag = self.getPositionListener()
                if self.position_flag == True:
                    rospy.loginfo("find a person, save the image")
                    pos_x = round(self.current_robot_pose["pos_x"], 1)
                    pos_y = round(self.current_robot_pose["pos_y"], 1)
                    address = "/home/lostm/Downloads/image/pic_"+ str(pos_x) + "_" + str(pos_y) + "_raw.jpg"
                    cv2.imwrite(address, self.out_img)
    ...
    def getPositionListener(self):
        try:
            dura_t = self.position_listener.getLatestCommonTime("/base_footprint", "/map")
            pos, ori = self.position_listener.lookupTransform("/map", "/base_footprint", dura_t)
            msg_dict = {
                "pos_x" : pos[0],
                "pos_y" : pos[1],
                "pos_z" : pos[2],
